      ...
      {!nestLabel ? (
        ...
        <div className="flex basis-full flex-col gap-1.5">
          <div className="flex gap-1.5">
            {payload[0]?.payload && getPayloadConfigFromPayload(config, payload[0], nameKey || payload[0].dataKey || "value")?.icon ? (\
              <getPayloadConfigFromPayload(config, payload[0], nameKey || payload[0].dataKey || "value").icon />
            ) : (
              !hideIndicator && (
                <div
                  className="h-2.5 w-2.5 shrink-0 rounded-[2px] border-[--color-border] bg-[--color-bg]"
                  style={{
                    "--color-bg": color || payload[0]?.color,
                    "--color-border": color || payload[0]?.color,
                  }}
                />
              )
            )}
